...
def power_eq(h, list):
    test_negatives = h - h0_initial

    if test_negatives.any() < 0:
       h = np.where(test_negatives < 0, h, h0_initial)
    valid_h = h - h0_initial
    if np.any(valid_h <= 0):
       print(valid_h[np.where(valid_h < 0)])
       raise ValueError("Invalid h - h0_initial: Some depths are too small for the chosen h0_initial.")

    curve_eq = 0
    for i in list:
        curve_eq += i + h ** i
    return curve_eq
    # return a + b * h + c * h ** 2 + d * h ** 3 + e * h ** 4 + f * h ** 5 + g * h ** 6 + i * h ** 7 + j * h ** 8 + k * h ** 9  # a * np.power((h - h0_initial), b)
...
def fit_power_eq():
    ...
    fitted_discharges = power_eq(ddho, *popt)

    # Plot the results
    plt.scatter(ddho, diso, label="Observed Data", color="blue", alpha=0.2)
    plt.plot(ddho, fitted_discharges, label="Fitted Power Equation", color="red")
    plt.xlabel("Depth (h)")
...
def rmse(sim, obs):
    return np.sqrt(np.mean(np.power((sim - obs) , 2)))
...
# maximalfehler 1mm = maximale absolute differenz , nicht mean
    '''
    np.random.seed(123)
