...
if __name__ == "__main__":
    ...
    df_change_value_inputs['change_value_inputs'] = ['1.' + str(x)[1:].replace('.', '').replace(',', '')  if str(x).startswith('1') else x for x in df_change_value_inputs['change_value_inputs']]
    df_change_value_inputs['change_value_inputs'] = df_change_value_inputs['change_value_inputs'].astype('float')
    ...
    df_task1_output = pd.read_csv(main_dir / 'task_1' / 'output_one_per_iteration_tol_0.01_seed_123.csv', header=None)
